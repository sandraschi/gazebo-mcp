...
const BACKEND_NAME: &str = "gazebo-mcp-backend.exe";
const BACKEND_PORT: u16 = 10991;
const BACKEND_TAG: &str = "gazebo-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "GAZEBO_MCP_PORT";
const ENV_HOST: &str = "GAZEBO_MCP_HOST";
const ENV_TAURI: &str = "GAZEBO_MCP_TAURI";
...
fn free_port(port: u16) -> bool {
    ...
    {
        let img_kill = format!(
            "Stop-Process -Name 'gazebo-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'gazebo-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM gazebo-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM gazebo-mcp-native.exe /T 2>$null"
        );
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name gazebo-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM gazebo-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
