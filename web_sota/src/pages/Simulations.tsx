...
export default function Simulations() {
  ...
  const handleAnalyze = async (jobId: string, worldName: string) => {
    ...
    const r = await fetch("/api/llm/chat", {
      ...
      body: JSON.stringify({
        model: "llama3.2:3b",
        prompt: `Analyze this Gazebo simulation state for world "${worldName}":\n${stateStr}\n\nProvide a brief summary of what's happening.`,
      }),
    });
      ...
      <div className="bg-slate-800 rounded-xl p-5 border border-slate-700 mb-6">
        ...
        <div className="flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-48">
            <label className="block text-xs text-slate-400 mb-1">World</label>
            <select
            ...
            >
              ...
              {Object.entries(worlds).map(([name]) => (
        ...
        <div className="divide-y divide-slate-700">
          ...
          {jobs.map((job) => (
            <div key={job.job_id}>
              ...
              >
                <div>
                  <span className="font-medium">{job.world_name}</span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {job.running && (
                    <>
                      <button
                        onClick={(e) => { e.stopPropagation(); handleAnalyze(job.job_id, job.world_name); }}
