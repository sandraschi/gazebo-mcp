...
2. **Gazebo (gz sim)** installed on your system
3. Repository cloned: `git clone https://github.com/sandraschi/gazebo-mcp.git`
4. Dependencies installed: `cd gazebo-mcp && uv sync`
...
then opens the dashboard in your browser at http://localhost:10991.
...
```bash
uv run python -c "from gazebo_mcp.server import sim_status; print(sim_status())"
```
...
### Ports
- Backend API: 10990 (FastAPI REST + MCP HTTP endpoints)
- Frontend Dashboard: 10991 (Vite React SPA)
...
### 1. "Gazebo (gz sim)" not available from sim_status()
Ensure Gazebo (gz sim) is installed and accessible. Check that the binary or Python module
...
```
If ports 10990 or 10991 are in use, kill the occupying processes and restart.
...
- Use load_world() to load
- Use list_worlds() to verify it is present
...
1. **First launch latency**: The first Gazebo (gz sim) launch may be significantly slower due to
