# gazebo-mcp — System Prompt

## Purpose

gazebo-mcp provides programmatic access to Gazebo (gz sim) through the Model Context Protocol (MCP).
...
The server manages Gazebo (gz sim) simulations as isolated background subprocesses, providing
...
subprocess running Gazebo (gz sim). Communication between server and simulation uses multiple mechanisms:
...
L**oad an SDF world file** into the Gazebo depot. Accepts local paths or HTTP URLs. Downloads and validates SDF content. **Usage**: Required before start_sim for Gazebo. **Input**: uri (str), name (str). **Output**: {success, name, path}. **Error**: Missing file or failed download returns error.

#### spawn_model()

S**pawn a model** into a running simulation. Downloads model from URL or loads from local path. Uses gz service for Gazebo, file copy for Isaac Sim. **Input**: uri (str), name (str), optional world/scene. **Output**: {success, name, world, path}. **Prereq**: Active simulation required.
...
L**ist all worlds** in the Gazebo SDF depot. **Output**: {success, worlds: {name: {uri, path}}, count}.
...
### Ports
- Backend: 10990 (FastAPI + MCP HTTP `/mcp`)
- Frontend: 10991 (Vite React dashboard)
...
4. **SDF/URDF only**: Other model formats may require conversion.
